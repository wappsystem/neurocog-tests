...
m.load=function(){
    load2();
    identification=""
    if(m.input.ID!="" && m.input.ID!=undefined) identification=m.input.ID;
    output_file="NBack output"
    if(m.input.file!="" && m.input.file!=undefined) output_file=m.input.file;
    $('#header').hide();
    $('#footer').hide();
...
var data_process=function(data){
    ...
    output[0].ID=identification;
    //console.log(output);
    output_file=output_file+".csv"
    $vm.download_csv(output_file,output);
    $('#header').show();
    $('#footer').show();
    window.history.go(-1);
    //alert(JSON.stringify(data));
